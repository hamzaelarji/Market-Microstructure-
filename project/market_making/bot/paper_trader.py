...
Usage (from project root):
    python -m market_making.bot.paper_trader --symbol BTCUSDT --gamma 1e-6 --T 3600
    python -m market_making.bot.paper_trader --simulated
"""
...
from market_making.core.intensity import delta_star, fill_prob
...
class PaperTrader:
    ...
    def process_tick(self, mid_price, dt):
        ...
        # Simulate fills — uses fill_prob() from core/intensity
        if db < np.inf and n < self.Q:
            lam = self.A * np.exp(-self.k * max(db, 0.0))
            if self._rng.uniform() < fill_prob(lam, dt):
